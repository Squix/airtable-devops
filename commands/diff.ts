...
export const diff_schema_command = new Command()
  ...
  .option(
    "-f, --tracked-file <file:string>",
    "Path to the schema file inside a git repository.",
    {
      depends: ["git"],
    }
  )
  ...
  .option("--color", "Use colors in the output", { default: true })
  .action(diff);
...
export async function diff(options: {
  ...
  try {
    ...
    if (options.git) {
      ...
      oldSchemaText = await Deno.readTextFile(options.old);
      newSchemaText = await Deno.readTextFile(options.new);
    }
...
async function gitShow(commit: string, trackedFile: string): Promise<string> {
...
function formatFieldChanges(oldField: Field, newField: Field): string {
  ...
  if (oldField.options || newField.options) {
    ...
    for (const [key, newValue] of Object.entries(newOptions)) {
      if (
        key in oldOptions &&
        JSON.stringify(oldOptions[key]) !== JSON.stringify(newValue)
      ) {
        changes.push(
          `options: ~ ${key}: ${JSON.stringify(
            oldOptions[key]
...
function formatTableDiff(tableDiff: TableDiff): string {
  const symbol =
    tableDiff.type === "CREATE" ? "+" : tableDiff.type === "DELETE" ? "-" : "~";
  const tableName = tableDiff.table.name;
  const typeText =
    tableDiff.type === "CREATE"
      ? "NEW TABLE"
      : tableDiff.type === "DELETE"
      ? "REMOVED TABLE"
      : "MODIFIED TABLE";
